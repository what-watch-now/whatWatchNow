...
router.get("/detail", (req, res, next) => {
  Movie.findOne({ netflixid: req.query.netflix_id })
    .then(theMovie => {
      ...
      if (req.user) {
        if (req.user.favList.includes(theMovie._id)) favouriteAdded = true;
        if (req.user.blackList.includes(theMovie._id)) blackAdded = true;
        if (req.user.viewList.includes(theMovie._id)) viewAdded = true;
      }
